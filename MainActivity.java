...
(function() {
  // Inject the high-finesse Apple squircle pulsing highlights and self-scaling container styles
  ...
  // Monitor details toggle events natively to advance step 5 and step 7
  document.addEventListener('toggle', (e) => {
    ...
    if (details.tagName && details.tagName.toLowerCase() === 'details') {
      ...
      if (summaryText === 'Voice Triggers' && details.open && window.tutStep === 4) {
        window.openTutorial(5);
      } else if (summaryText === 'Advanced Settings' && details.open && window.tutStep === 6) {
        window.openTutorial(7);
      }
    }
  }, true);

  // Watch for volume slider interactions natively to advance step 4
  document.addEventListener('input', (e) => {
    if (window.tutActive && window.tutStep === 3) {
      if (e.target.closest('#cue-volume-slider-row input[type="range"]')) {
        window.openTutorial(4);
      }
    }
  });

  // Watch for dynamic track container changes to advance step 1 [1]
  const trackObserver = new MutationObserver(() => {
    if (window.tutActive && window.tutStep === 0 && window.tracks && window.tracks.length > 0) {
      window.openTutorial(1);
    }
  });

  // Initialize observers and event listeners on window load
  window.addEventListener('load', () => {
    injectTutorialHTML();
    
    // Register dialog open/close lifecycle observer [1]
    ...
    // Start watching tracklist container
    const trackListEl = document.getElementById('track-list');
    if (trackListEl) {
      trackObserver.observe(trackListEl, { childList: true });
    }
